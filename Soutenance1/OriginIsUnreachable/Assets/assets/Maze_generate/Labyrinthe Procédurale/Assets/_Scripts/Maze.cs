...
public class Maze : MonoBehaviour {
    ...
    void CreateWalls()
    {
        ...
        for (int i = 0; i< ySize; i++)
        {
            ...
            {
                myPosition = new Vector3(initialPosition.x + (j * wallLength) - wallLength / 2, 0.0f, initialPosition.z + (i * wallLength) - wallLength / 2);
        ...
        for (int i = 0; i <= ySize; i++)
        {
            for (int j = 0; j < xSize; j++)
            {
                myPosition = new Vector3(initialPosition.x + (j * wallLength), 0.0f, initialPosition.z + (i * wallLength) - wallLength);
    ...
    void CreateCells()
    {
        ...
        for(int cellprocess = 0; cellprocess < cells.Length; cellprocess++)
        {
            ...
            cells[cellprocess].south = allWalls[childrenProcess + (xSize + 1) * ySize];
            eastwestProcess++;
            termCount++;
            childrenProcess++;
            cells[cellprocess].west = allWalls[eastwestProcess];
            cells[cellprocess].north = allWalls[(childrenProcess + (xSize + 1) * ySize)+xSize-1];
        }
        CreateMaze();
    }
